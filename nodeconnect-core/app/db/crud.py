...
def autodelete_inactive_users(db: Session, days: int) -> List[User]:
    """
    Deletes users who have been inactive (no connection) for a specified number of days.

    Args:
        db (Session): Database session
        days (int): Number of days of inactivity before deletion.
    ...
    query = db.query(User).options(joinedload(User.admin))
    inactive_users = []
    now = datetime.utcnow()
    
    for user in query:
        # If never online, use created_at. If somehow both are None, skip.
        reference_time = user.online_at if user.online_at else user.created_at
        if reference_time and reference_time + timedelta(days=days) <= now:
            inactive_users.append(user)

    if inactive_users:
        remove_users(db, inactive_users)
