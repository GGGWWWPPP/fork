...
const ActionButtons: FC<ActionButtonsProps> = ({ user }) => {
  ...
  const handleResetHWID = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    try {
      const res = await fetch(`/api/user/${user.username}/hwids`, { method: "DELETE" });
      if (res.ok) {
        toast({ title: "HWIDs Reset Successfully", status: "success", isClosable: true });
      } else {
        toast({ title: "Failed to reset HWIDs", status: "error", isClosable: true });
      }
    } catch (err) {
      toast({ title: "Error resetting HWIDs", status: "error", isClosable: true });
    }
  };
  ...
  return (
    ...
    >
      ...
      {user.hwid_limit > 0 && (
        <Tooltip label="Reset HWIDs" placement="top">
          <IconButton
            p="0 !important"
            aria-label="reset hwids"
            bg="transparent"
            _dark={{ _hover: { bg: "gray.700" } }}
            size={{ base: "sm", md: "md" }}
            onClick={handleResetHWID}
          >
            <chakra.svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" w="5" h="5">
