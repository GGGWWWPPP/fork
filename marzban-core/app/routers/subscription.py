...
def user_subscription(
    ...
    # HWID Limit Check
    if dbuser.hwid_limit > 0 and x_hwid:
        from app.db import hwid as hwid_db
        user_hwids = hwid_db.get_user_hwids(db, dbuser.id)
        if not any(h.hwid_value == x_hwid for h in user_hwids):
            if len(user_hwids) >= dbuser.hwid_limit:
                from fastapi import HTTPException
                raise HTTPException(status_code=403, detail="Device Limit Exceeded")
            hwid_db.add_user_hwid(db, dbuser.id, x_hwid)
