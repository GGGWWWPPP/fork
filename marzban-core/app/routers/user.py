...
@router.get("/user/{username}/hwids", responses={403: responses._403, 404: responses._404})
def get_user_hwids(
    db: Session = Depends(get_db),
    dbuser: UserResponse = Depends(get_validated_user),
    admin: Admin = Depends(Admin.get_current),
    ...
    """Get user HWIDs"""
    from app.db.hwid import get_user_hwids
    hwids = get_user_hwids(db, dbuser.id)
    return {"hwids": [h.hwid_value for h in hwids]}


@router.delete("/user/{username}/hwids", responses={403: responses._403, 404: responses._404})
def reset_user_hwids(
    db: Session = Depends(get_db),
    dbuser: UserResponse = Depends(get_validated_user),
    admin: Admin = Depends(Admin.get_current),
):
    """Reset user HWIDs"""
    from app.db.hwid import reset_user_hwids
    reset_user_hwids(db, dbuser.id)
    logger.info(f'User "{dbuser.username}" HWIDs reset')
    return {"detail": "User HWIDs successfully reset"}
